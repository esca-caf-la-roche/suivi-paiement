import { useAuth } from '../contexts/AuthContext'

export default function HomePage() {
  const { responsible, signOut } = useAuth()

  return (
    <div className="min-h-screen bg-blanc font-sans flex items-center justify-center p-8">
      <div className="border-4 border-noir shadow-[8px_8px_0px_#000000] bg-blanc p-10 max-w-sm w-full">

        <span className="inline-block bg-citron border-2 border-noir text-noir text-xs font-bold uppercase tracking-widest px-3 py-1 mb-6">
          CAF La Roche-Bonneville
        </span>
        <h1 className="text-3xl font-black text-noir leading-tight mb-2">
          FJD Inscriptions
        </h1>
        <p className="text-base font-semibold text-noir border-l-4 border-glace pl-3 mb-8">
          Connecté ✓{responsible ? ` — ${responsible.name}` : ''}
        </p>

        <ul className="space-y-1 text-sm text-noir font-mono border-t-2 border-noir pt-4 mb-8">
          <li>→ Auth OTP email ✓</li>
          <li>→ RLS actif ✓</li>
          <li>→ Phase 2 complète</li>
          <li className="text-noir/40">→ Page Validation (Phase 6)</li>
          <li className="text-noir/40">→ Page Config (Phase 5)</li>
        </ul>

        <button
          onClick={signOut}
          className="w-full border-2 border-noir text-noir font-bold uppercase tracking-widest text-xs px-4 py-2 hover:bg-noir hover:text-blanc transition-colors"
        >
          Se déconnecter
        </button>
      </div>
    </div>
  )
}
